...
\begin{thm}\label{thm:rate_doubling1} Let $\Omega$ be an open, bounded and connected subset $\mathbb{R}^n$ with $\mathrm{C}^2$ boundary. Assume $\mathrm{(A1)},\mathrm{(H1)}-\mathrm{(H4)}$ and $1<p<2$. Let $u^\varepsilon$ be the unique solution to \eqref{eq:PDEeps} and $u^0$ be the unique solution to \eqref{eq:PDE0}, then there exists a constant $C$ independent of $\varepsilon$ such that
\begin{equation*}
...
\end{equation} 
Let $C_f$ be the Lipschitz constant of $f$ and $\beta,\sigma,\delta \in (0,1)$ to be chosen later. Recall the definition of $C_\varepsilon$ from Theorem \ref{thm:wellposed1<p<2}, we define the auxiliary functional
\begin{align*}
    \Phi^{\sigma}(x,y) =  u^\varepsilon(x) - u_\delta(y) - \frac{C_0|x-y|^2}{\sigma}- \frac{C_\varepsilon}{d(x)^{\alpha+\beta}}, \qquad (x,y)\in \overline{\Omega}\times\overline{\Omega}_\delta,
\end{align*}
where $C_0$ is the Lipschitz constant of $u^0$ (also the Lipschitz constant of $u_\delta$ for $\delta>0$). We split the proof into several steps.
\begin{enumerate}
...
\color{blue}
\item[2.] Assume that $x_\sigma\notin \overline{\Omega}_\delta$, i.e., $d(x_\sigma) < \delta$. Let $\underline{x}_\sigma\in \partial\Omega_\delta$ be the projection of $x_\sigma$ onto $\partial\Omega_\delta$, then $d(\underline{x}_\sigma) = \delta$ and for all $z\in \partial\Omega$ then
\begin{equation*}
    |\underline{x}_\sigma - x_\sigma| \leq C\delta. \qquad (\color{red} C<1) \color{blue}
\end{equation*}
Now $\Phi^\sigma(x_\sigma,y_\sigma) \geq \Phi^\sigma(x_\sigma,\underline{x}_\sigma)$ gives us that
    \begin{align}\label{e:est_sigma2-new}
        \frac{C_0|x_\sigma-y_\sigma|^2}{\sigma} &\leq \frac{C_0|x_\sigma-\underline{x}_\sigma|^2}{\sigma} +  u_\delta(\underline{x}_\sigma) - u_\delta(y_\sigma)  \nonumber \\
        &\leq  \frac{C_0|x_\sigma-\underline{x}_\sigma|^2}{\sigma} + C_0|\underline{x}_{\sigma} - y_\sigma|.
    \end{align}
    Therefore
    \begin{align*}
        |x_\sigma - y_\sigma|^2 &\leq \sigma|\underline{x}_\sigma - y_\sigma| + |x_\sigma - \underline{x}_{\sigma}|^2 \\
        &\leq \sigma|\underline{x}_\sigma - x_\sigma| + \sigma|x_\sigma - y_\sigma| + |x_\sigma - \underline{x}_\sigma|^2\\
        &\leq \sigma|x_\sigma - y_\sigma| + \delta \sigma + \delta^2\leq \frac{|x_\sigma - y_\sigma|^2}{2} + 2\sigma^2 + \delta\sigma +\delta^2.
    \end{align*}
    Therefore
    \begin{equation*}
        |x_\sigma - y_\sigma|^2 \leq 4\sigma^2 + 2\delta\sigma + 2\delta^2
    \end{equation*}
    Let $\delta = \theta \sigma$ for $\theta > 1$, then
    \begin{equation*}
        |x_\sigma - y_\sigma|^2\leq 2\sigma^2(\theta+2)^2
    \end{equation*}
\color{black}
\item[2.] $\Phi^\sigma(x_\sigma,y_\sigma) \geq \Phi^\sigma(x_\sigma,x_\sigma)$ gives us that
    \begin{equation}\label{e:est_sigma2}
     \frac{C_0|x_\sigma-y_\sigma|^2}{\sigma} \leq u_\delta(x_\sigma) - u_\delta(y_\sigma) \leq C_0|x_\sigma-y_\sigma| \qquad\Longrightarrow\qquad |x_\sigma -y _\sigma|\leq \sigma.
    \end{equation}
    This gives us that
    \begin{equation}\label{e:est_sigma3}
        d(x_\sigma) \geq d(y_\sigma) - |x_\sigma-y_\sigma|\geq \delta-\sigma.
    \end{equation}
    Let $\delta = 2\sigma$, we deduce that $d(x_\sigma)\geq \sigma$.
    \item[3.] $x\mapsto \Phi^\sigma(x,y_\sigma)$ has a maximum at $x_\sigma\in \Omega$, thus by subsolution test for \eqref{eq:PDEeps} we have
\begin{align}\label{e:sub1}
    &\lambda u^\varepsilon(x_\sigma) + \left|\frac{2C_0(x_\sigma-y_\sigma)}{\sigma} - \frac{C_\varepsilon(\alpha+\beta)}{d(x_\sigma)^{\alpha+\beta+1}}D d(x_\sigma )\right|^p - f(x_\sigma)\nonumber\\
    &\qquad\qquad -\varepsilon\left(\frac{2n}{\sigma} + \frac{C_\varepsilon(\alpha+\beta)(\alpha+\beta+1)|D d(x_\sigma)|^2}{d(x_\sigma)^{\alpha+\beta+2}}-\frac{C_\varepsilon(\alpha+\beta)}{d(x_\sigma)^{\alpha+\beta+1}}\Delta d(x_\sigma)\right)  \leq 0.
\end{align}
\item[4.] Let
\begin{equation}\label{e:est_xi_zeta}
    \xi_\sigma = \frac{2C_0(x_\sigma-y_\sigma)}{\sigma} \qquad\text{and}\qquad \zeta_\sigma = - \frac{C_\varepsilon(\alpha+\beta)}{d(x_\sigma)^{\alpha+\beta+1}}D d(x_\sigma)
\end{equation}
then $|\xi_\sigma|\leq 2C_0$ by \eqref{e:est_sigma2} and furthermore $\xi_\sigma+\zeta_\sigma \in D^-u^\varepsilon(x_\sigma)$, which, by the local gradient estimate, as $x_\sigma\in \Omega_{\sigma}$, $\xi_\sigma+\zeta_\sigma$ is bounded in terms of $\sigma$ and will blow up as $\sigma\to 0$. Nevertheless, we can do better as following.
\begin{align*}
    |\zeta_\sigma|=\left|\frac{C_\varepsilon(\alpha+\beta)}{d(x_\sigma)^{\alpha+\beta+1}}D d(x_\sigma )\right| \leq \frac{C_\varepsilon(\alpha+\beta)}{\sigma^{\alpha+\beta+1}} 
