...
\end{equation*}
for $x\in \Omega$.
\item Idea to bound point-wise $u^\varepsilon(x)$ without using stochastic formula. If $x\in \Omega$ such that we can find $r>0$ such that $B(x,r)\subset \{f = 0\}$ then by consider an explosive solution $w^\varepsilon$ in $B(x,r)$, then as $w^\varepsilon\to 0$ in $B(x,r)$ and $u^\varepsilon \leq w^\varepsilon$ in $B(x,r)$, one can get a rate locally for $u^\varepsilon$ if we can control the radius $r$, provided we know the support of $f$. \textcolor{red}{This idea is clear, probably no more explanation needed or further investigation, maybe?}
