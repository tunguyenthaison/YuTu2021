...
Tasks to try:
\begin{itemize}
    \item[1.] Getting another term in
    \begin{equation*}
        u^\varepsilon(x) - \frac{C_\alpha\varepsilon^{\alpha+1}}{d(x)^{\alpha}} \approx ?
    \end{equation*}
    \item[2.] For $p>2$, review well-posedness for 
    \begin{equation*}
        \begin{cases}
            \lambda u + |Du|^p - f(x) \leq \varepsilon \Delta u \qquad
             \text{in}\;\Omega,\\
             \lambda u + |Du|^p - f(x) \geq \varepsilon \Delta u \qquad
             \text{on}\;\overline{\Omega}.
        \end{cases}
    \end{equation*}
    Review that $u\in \mathrm{C}(\overline{\Omega})$.
    \item[3.] Understand the going from parabolic (existence of Green function) to elliptic. 
\end{itemize}
